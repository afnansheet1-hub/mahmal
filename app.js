...
let uptExcludedQuantityTotal = 0;
...
async function analyzePdf(source, name) {
  ...
  uptExcludedQuantityTotal = rows
    .filter((row) => isOfferDiscount(row) || !row.barcode || !row.product)
...
function renderDailyExtract({ products, countProducts, totalSales, totalQty, date }) {
  ...
  const uptBaseQty = Math.max(0, pdfQuantityTotal - uptExcludedQuantityTotal);
